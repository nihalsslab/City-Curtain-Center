...
function getProducts() {
  const rows = getSheetData(TABS.PRODUCTS);
...
function getStats() {
  ...
  // Sales -> Income
  sales.forEach(r => addVal(r[1], 'inc', r[8]));
...
function uploadImage(base64Data, mimeType, fileName) {
  ...
  // Return a direct link or thumbnail link
  // Using the export=view format is often reliable for <img> tags
