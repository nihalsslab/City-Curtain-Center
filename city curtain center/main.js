  ...
  const cards = state.products.map(p => `
    <div class="product-card">
      <div class="product-image">
        ${p.imageUrl ? `<img src="${p.imageUrl}" style="max-width:100%; max-height:100%;">` : '<span class="material-icons-round" style="font-size:48px; color:#ddd;">image</span>'}
      </div>
  ...
  const historyRows = state.sales.slice(-10).reverse().map(s => {
    ...
    <div style="border-bottom:1px solid #eee; padding:0.8rem 0; display:flex; gap:10px; align-items:center;">
      ...
      <div style="text-align:right;">
         ...
         ${isAdv ? '<div style="font-size:0.75rem; color:#e67e22;">Advance</div>' : ''}
      </div>
    </div>
  `}).join('');
